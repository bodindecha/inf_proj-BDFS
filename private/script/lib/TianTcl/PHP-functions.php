<?php
	...
	// Shorten
	if (!function_exists("sha256")) { function sha256($str, bool $binary=true): string {
		return hash("sha256", $str, $binary);
	} }

	// PHP < 8
	if (!function_exists("str_contains")) { function str_contains(string $haystack, string $needle): bool {
		return ($needle <> "" && mb_strpos($haystack, $needle) <> false);
	} }

	// PHP < 5.4
	if (!function_exists("hex2bin")) { function hex2bin($str) {
		$map = array(
		...
		$length = strlen(trim(strval($str)));
		if ($length % 2 <> 0) {
			user_error("Hexadecimal input string must have an even length", E_USER_WARNING);
			return false;
		} if (strspn($str, "0123456789ABCDEFabcdef") <> $length) return false;
