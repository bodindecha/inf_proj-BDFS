<?php
	...
	if (!isset($_REQUEST["path"]) || !strlen($_REQUEST["path"])) TianTcl::http_response_code(902);
	$path = ltrim(trim($_REQUEST["path"]), "/");
	...
	// Block direct access
	define("SESSVAR_KEY", "image_token");
	function genViewToken() {
		return base_convert(crc32(date("H:i:").(string)floor((int)date("s")/1.5)), 10, 36);
	}
	$printKey = TianTcl::encrypt("print");
	$printRandom = crc32(date("H:").floor((int)date("i")/5));

	$hrfr = $_SERVER["HTTP_REFERER"] ?? "";
	$requestURI = explode("?", $_SERVER["REQUEST_URI"])[0];
	$isForced = (isset($_REQUEST["token"]) && $_REQUEST["token"] == TianTcl::sessVar(SESSVAR_KEY));
	$isPrint = (isset($_REQUEST[$printKey]) && $_REQUEST[$printKey] == $printRandom);
	if (
		(
			strlen($hrfr) &&
			parse_url($hrfr, PHP_URL_PATH) <> "$APP_CONST[baseURL]$path"
		) ||
		(
			in_array($requestURI, [$APP_CONST["baseURL"].$path, $APP_CONST["baseURL"]."_resx/service/view/image"]) &&
			(
				($isForced && !preg_match("/(^|&)token=[0-9a-z]+(&|$)/", parse_url($hrfr, PHP_URL_QUERY))) ||
				($isPrint && !preg_match("/(^|&)$printKey=[0-9a-z]+(&|$)/", parse_url($hrfr, PHP_URL_QUERY)))
			)
		)
	) { // Pass real imaage
		if ($isForced) TianTcl::sessVar(SESSVAR_KEY, genViewToken());
		TianTcl::sendFile($real);
	} // Parse viewer
	TianTcl::sessVar(SESSVAR_KEY, genViewToken());
	$loadLink = "$APP_CONST[baseURL]$path?token=".TianTcl::sessVar(SESSVAR_KEY);

	$size = getimagesize($real);
	$header["title"] = "$name ($size[0]×$size[1]) - Image viewer";
	$header["cover"] = $loadLink;
	# echo "hrfr: $hrfr<br>c1: "."$_SERVER[REQUEST_SCHEME]://$_SERVER[SERVER_NAME]$APP_CONST[baseURL]$path"."<br>ruri: $requestURI<br>c2: ".$APP_CONST["baseURL"].$path."<br>c3: ".$APP_CONST["baseURL"]."_resx/service/view/image<br>iF: ".($isForced?"T":"F");
	$APP_PAGE -> print -> head();
?>
<style type="text/css">
	main {
	...
	main .container .wrapper span {
		background-image: url("<?=$loadLink?>"); background-size: contain;
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		app.IO.URL.removeQuery(null, "token=");
	});
	...
	function print_file(me) {
		...
		printJS({
			printable: atob("<?=rtrim(base64_encode("$APP_CONST[baseURL]$path?$printKey=$printRandom"), "=")?>"),
			type: "image",
