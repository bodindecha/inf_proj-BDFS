<?php
	...
	require_once("php/core/reload_settings.php");
	// Find files
	if (isset($_GET['mode'])) {
		if ($_GET['mode']=="PBL" && isset($_GET['file']) && isset($_GET['code'])) {
			if (!isset($_SESSION['auth'])) header("Location: /$my_url");
			$base_path = "upload/PBL/".$_SESSION['stif']['t_year']."_".$_GET['code']."-".$_GET['file'];
			foreach (array("pdf", "png", "jpg", "jpeg") as $ft) {
				if (file_exists("$base_path.$ft")) {
					$type = $ft; $path = "/resource/$base_path.$type";
					$download = "PBL - ".$_GET['file'].".$type";
					$size_fetch = "$base_path.$type";
					break;
			} } $found = isset($type);
		} else if ($_GET['mode']=="Manual" && isset($_GET['doc'])) {
			if (!isset($_SESSION['auth'])) header("Location: /$my_url");
			$home_menu = "manual"; $is_std = $_SESSION['auth']['type']=="s";
			# if (!in_array($_GET['doc'], array("PBL"))) {
				/* $base_path = "upload/Manual/"; $type = "pdf"; $file = trim($_GET['doc'])."-".($is_std ? "student": "teacher");
				$path = "/resource/$base_path$file.$type"; # $download = "คู่มือการใช้งานระบบ $file.$type";
				$size_fetch = "$base_path$file.$type";
				$found = file_exists($size_fetch); */
				$file = ($is_std ? "1jB1k-HkulOishLZ4pxhqyBHeHHFpFv1e": "1mcywgRs5kXcTYgbWYPmXMMIWyhyldJLT");
				header("Location: https://drive.google.com/a/bodin.ac.th/file/d/$file/preview"); exit(0); /*
			} else { switch($_GET['doc']) {
					case "PBL": $source = ($is_std ? "2PACX-1vTGvGn1ly6JDmIw8EPYxP8fQRJGe_rZh7pEBbJqcKD8g1qGKZCDlBh9pUogJSyFlhZlDQcc7YY1NwH-": "2PACX-1vSZgOWh4i-evCsosCqfMUzDsu6qAwWbK2dc0XSa5QZkM3hEaRx0dtrrn6ch9auDFfrWdSufTfvmpATw"); break;
					default: $source = ($is_std ? "": ""); break;
				} $found = (isset($source) && $source<>"");
				$type = "ggDoc";
			} */
		} else if ($_GET['mode']=="file-share" && isset($_GET['file'])) {
			# if (!isset($_SESSION['auth'])) header("Location: /$my_url");
			$file = base64_decode(trim($_GET['file'])."==");
			$path = "/service/app/file-share/$file"; $base_path = "..$path"; $type = "";
			$size_fetch = $base_path;
			$found = file_exists("$base_path");
			$nodownload = true;
		} else $found = false;
	} else $found = false;

	// Other codes
	$size = array(); list($size[0], $size[1], $size[2], $size[3]) = getimagesize($size_fetch);
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<?php require("hpe/heading.php"); require("hpe/init_ss.php"); ?>
		<style type="text/css">
			html body main { min-height: calc(var(--window-height) - var(--top-height) - 4px); height: calc(var(--window-height) - var(--top-height) - 4px); }
			html body.nohbar main { min-height: calc(var(--window-height) - 4px); height: calc(var(--window-height) - 4px); overflow: hidden; }
			...
			html body main div.container div.wrapper span {
				background-image: url("<?php echo $path; ?>"); background-size: contain;
			...
			@media only screen and (max-width: 768px) {
				...
				html body main div.controller div.bar ul { padding: 0px 1.25px; width: 325px; }
			...
			@media only print {
				...
				html body nav.rfr { display: none; }
			}
		</style>
		<script type="text/javascript">
			...
			function download(e) {
				...
				if (cooldownload.t==null) {
					...
					let getfileurl = "/resource/dl?furl=<?php echo urlencode(ltrim($path, "/")); ?>";
	...
	<body>
		<?php require("hpe/header.php"); ?>
		<main shrink="<?php echo($_COOKIE['sui_open-nt'])??"false"; ?>">
			<?php
				// Display File
				if ($found) {
					if ($type == "ggDoc") echo '<iframe src="https://docs.google.com/document/d/e/'.$source.'/pub?embedded=true">Loading...</iframe>';
					else if ($type == "pdf") echo '<iframe src="https://docs.google.com/viewerng/viewer?embedded=true&url=https%3A%2F%2Finf.bodin.ac.th'.str_replace("/", "%2F", $path).'">Loading...</iframe>';
					else {
							...
							<div class="controller">
								...
								<div class="bar"><ul>
									...
									<li>
										...
										<a onClick="dlbtnfxfix()" href="javascript:void(0)" '.(isset($nodownload)?"disabled":"").'><i class="material-icons">download</i><span data-title="Download (ctrl+D)"></span></a>
										<a class="adlder" download="'.($download??"").'" style="display: none;"></a>
									</li>
								</ul></div>';
						# header("Location: $path");
						// echo '<script type="text/javascript">location = "'.$path.'";</script>';
					}
				} else echo '<iframe src="/error/900">Loading...</iframe>';
			?>
			
		</main>
		<?php require("hpe/material.php"); ?>
		<footer>
			<?php require("hpe/footer.php"); ?>
		</footer>
	</body>
</html>